...
def create_sinus(folder, freq=1):
    "create the dias"
    pic_pr_period = PICTURE_SIZE/freq
    ...
    frq = "_" + str(int(freq))
    rgb.save(savefolder / ("sinus_c"+frq+".png"), dpi=DPIXY, exif=exif, pnginfo=metadata)
    rgba.save(savefolder / ("sinus_ca"+frq+".png"), dpi=DPIXY, pnginfo=metadata)
    grey.save(savefolder / ("sinus_bw"+frq+".png"), dpi=DPIXY, pnginfo=metadata)
    greya.save(savefolder / ("sinus_a"+frq+".png"), dpi=DPIXY, pnginfo=metadata)
    return rgb
